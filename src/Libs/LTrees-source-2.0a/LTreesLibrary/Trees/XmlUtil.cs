...
namespace LTreesLibrary.Trees
{
    ...
    public static class XmlUtil
    {
        ...
        public static float GetFloat(XmlNode child, String attributeName)
        {
            ...
            catch (FormatException e)
            {
                throw new ArgumentException(
                    String.Format("Attribute '{0}' set to '{1}' could not parse as a float; found in element '{2}'",
                                  attributeName, value, child.Name), e);
            }
        }
        ...
        public static int GetInt(XmlNode child, String attributeName)
        {
            ...
            {
                throw new ArgumentException(
                    String.Format("Attribute '{0}' set to '{1}' could not parse as a int; found in element '{2}'",
                                  attributeName, value, child.Name), e);
            }
        }
        ...
        public static string GetString(XmlNode child, string attributeName)
        {
            ...
            if (a == null)
                throw new ArgumentException(String.Format("Required attribute '{0}' not found in element '{1}'.",
        ...
        public static Color GetColor(XmlNode child, string attributeName)
        {
            ...
            if (parts.Length != 3)
                throw new ArgumentException(
                    String.Format("Attribute '{0}' set to '{1}' could not parse as a color; found in element '{2}'",
                                  attributeName, s, child.Name));
            try
            {
                byte r = byte.Parse(parts[0], NumberStyles.Any, numberFormat);
                byte g = byte.Parse(parts[1], NumberStyles.Any, numberFormat);
                byte b = byte.Parse(parts[2], NumberStyles.Any, numberFormat);
        ...
        public static Color GetColor(XmlNode child, string attributeName, Color defaultColor)
        {
            ...
            try
            {
                ...
                byte g = byte.Parse(parts[1], NumberStyles.Any, numberFormat);
                byte b = byte.Parse(parts[2], NumberStyles.Any, numberFormat);
                byte a = parts.Length > 3 ? byte.Parse(parts[3], NumberStyles.Any, numberFormat) : defaultColor.A;
        ...
        public static Vector2 GetVector2(XmlNode child, string attributeName)
        {
            ...
            if (parts.Length < 2)
                throw new ArgumentException(
                    String.Format("Attribute '{0}' set to '{1}' could not parse as a Vector2; found in element '{2}'",
                                  attributeName, s, child.Name));
            try
            {
        ...
        public static Vector3 GetVector3(XmlNode child, string attributeName)
        {
            ...
            if (parts.Length != 3)
                throw new ArgumentException(
                    String.Format("Attribute '{0}' set to '{1}' could not parse as a Vector3; found in element '{2}'",
                                  attributeName, s, child.Name));
            try
            {
                float x = float.Parse(parts[0], numberFormat);
