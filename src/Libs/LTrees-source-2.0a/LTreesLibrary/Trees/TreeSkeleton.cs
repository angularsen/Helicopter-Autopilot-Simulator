...
namespace LTreesLibrary.Trees
{
    ...
    {
        private readonly List<TreeBone> bones = new List<TreeBone>();
        private readonly List<TreeBranch> branches = new List<TreeBranch>();
        ...
        public float GetLongestBranching(float[] destinationArray)
        {
            ...
            for (int i = 0; i < branches.Count; i++)
            {
                ...
                if (branches[i].ParentIndex != -1)
                    dist += destinationArray[branches[i].ParentIndex] -
                            branches[branches[i].ParentIndex].Length*(1.0f - branches[i].ParentPosition);
                if (dist > maxdist)
                    maxdist = dist;
